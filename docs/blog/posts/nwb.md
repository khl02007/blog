...
You may have heard that NWB is a format for neuroscience data. But what does that mean? What *is* a data format? We can gain some intuition about this from formats that we are familiar with, such as JPEG for images and MP3 for audio. These formats are actually *rules for how the data is stored as 0s and 1s by the computer*. Once we know these rules, we can reverse them and read the file[^1]. But creating a format goes much beyond these rather dull technical details of how the data is stored as 0s and 1s. It has a huge *social* component. This is because a format is only as useful as its reach. The more widely used it is, the more likely it will be adopted by new users, since their data will be intelligible to more people. In that sense it is a bit like other phenomena that display a network effect, such as social media. 

Until recently, there was no format that was widely adopted by neuroscientists, making it problematic for the community to share data: with other labs, or within labs, or even with your future self. NWB was born to serve as the standard answer to this problem. But as I said, a format has to be widely adopted to be useful. And to achieve that, it has to accommodate everyone's[^2] needs--after all, why would you use it if it doesn't work for you? And given that neuroscientists are famous for doing all kinds of different stuff, this is a really hard social engineering problem that no amount of technical chops alone will solve. 
